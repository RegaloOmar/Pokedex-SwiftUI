...
public final class ServiceCoordinator {
    ...
    /**
     ...
     - Parameters:
        - timeOutRequest: The maximum timeout in seconds for an individual request (default: 20 seconds).
        - timeOutResource: The maximum timeout in seconds for the entire data transfer (default: 20 seconds).
        ...
        - httpMaximumConnectionsPerHost: The maximum number of simultaneous connections per host (default: 2).
    ...
    public func setConfiguration(
        timeOutRequest: Double = 20,
        timeOutResourse: Double = 20,
    ...
    public func sendRequest<T: Codable>(
        ...
        guard let url = getURL(url: urlString, parameters: params) else {
            return .failed(error: PokemonNetworkError.invalidURL)
        }
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                ...
                case 200:
                    if !data.isEmpty {
                        ...
                        if let dataD: T = data as? T {
                            ...
                            return .failed(error: PokemonNetworkError.decodingError)
                        }
                        
                    } else {
                        return .failed(error: PokemonNetworkError.invalidResponse)
                    }
                case 201..<204:
                ...
                case 300..<400:
                    return .information(code: httpResponse.statusCode)
                case 400:
                    return .failed(error: PokemonNetworkError.badRequest)
                case 401:
                    return .failed(error: PokemonNetworkError.unauthorized)
                case 404:
                    return .failed(error: PokemonNetworkError.notFound)
                case 500...599:
                    return .failed(error: PokemonNetworkError.serverError(statusCode: httpResponse.statusCode))
                default:
                    return .failed(error: PokemonNetworkError.otherCode(code: httpResponse.statusCode))
                }
            }
            
            return .failed(error: PokemonNetworkError.invalidResponse)
        } catch let error {
            return .failed(error: PokemonNetworkError.other(message: error.localizedDescription))
        }
    }
...
public enum ServiceStatus<T> {
    ...
    case failed(error: PokemonNetworkError)
